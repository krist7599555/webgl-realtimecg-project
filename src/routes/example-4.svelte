<script lang="ts">
  ...
  import { Matrix4, toRadians } from '@math.gl/core';
	...
	onMount(() => {
		...
		const scottPatternFrameBufferInfo = twgl.createFramebufferInfo(gl, [
			{ attachmentPoint: gl.COLOR_ATTACHMENT0, target: gl.TEXTURE_2D, level: 0, attachment: targetTexture }
		], targetTextureWidth, targetTextureHeight) // set size to apply view port
		...
		function drawCube({ aspect, texture, rotateX = 0, rotateY = 0 }) {
			twgl.setUniforms(programInfo, {
				...
				u_matrix: new Matrix4()
					.identity()
					.multiplyRight(new Matrix4().perspective({ fovy: toRadians(60), aspect, near: 1, far: 2000 }))
